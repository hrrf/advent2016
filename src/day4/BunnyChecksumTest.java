 ...
 */
public class BunnyChecksumTest {
    @Test
    public void isLegit() throws Exception {

    }

    @Test
    public void shiftName() throws Exception {
        String s  = "aaaaa-bbb-z-y-x-1[abxyz]";
        BunnyChecksum bc = new BunnyChecksum(s);
    ...
    public void createCheckSum() {
        ...
        assertEquals(123, bc.sectorId.intValue());
        assertEquals( "abxyz", bc.checksum);

        String s2 = "not-a-real-room-404[oarel]";
        BunnyChecksum bc2 = new BunnyChecksum(s2);

        assertEquals(404, bc2.sectorId.intValue());
        assertEquals( "oarel", bc2.checksum);
    }

    @Test
    public void passChecksum() throws Exception {
        String s  = "aaaaa-bbb-z-y-x-123[abxyz]";
        BunnyChecksum bc = new BunnyChecksum(s);
        assertTrue(bc.isLegit());

        String s2 = "not-a-real-room-404[oarel]";
        BunnyChecksum bc2 = new BunnyChecksum(s2);
        assertTrue(bc2.isLegit());
    }

    @Test
    public void doTheThingWithTheThing() {
        String s = "bkwzkqsxq-tovvilokx-nozvyiwoxd-172[fstek]\n" +
        ...
        String[] blllll = s.split("\n");

        int count = 0;

        for(int i = 0; i < blllll.length; i++) {
            BunnyChecksum b = new BunnyChecksum(blllll[i]);

            if (b.isLegit()) {
                count += b.sectorId.intValue();

                String decoded = b.shiftName();
                if (decoded.contains("orth")) {
                    System.out.println("Decoded: [" + b.sectorId + "] : " + b.shiftName());
                }
            }
        }

        System.out.println("Sectorid sum:" + count);
    }
}